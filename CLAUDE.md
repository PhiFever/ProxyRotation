...
- **失败自愈按来源分流**（成本模型驱动，不是可有可无的优化）：
  ...
  - **但"探通就不切"必须有上界**：`api` / `command` 分支同样累加 `consecutiveFails`，连续到 `failThreshold` 就**无条件切，不再问探测**。探测答的是"这个代理还连得上 `test_url` 吗"，答不了"它还能不能用来连目标"——目标把出口 IP 标记掉时（回验证页、或直接拒连），代理对代理商自家端点仍然通，探测永远判活，自愈再也触发不了，同一个上游会一直失败到自己过期。2026-08-13 真机实测：一个刚经 `/switch` 买来的 IP 被目标连拒 8 次、卡住 19 秒，采集侧掉速 24→15 页/分。**这个洞只能在 Rotator 里补**：调用方拿到的只有一个连接错误，分不出"IP 到期"（自愈会换，插手就是多买一个）还是"IP 被标记"（必须换），只有 Rotator 同时握着「连续失败」与「探测通过」两个事实。回归保护是 `TestMarkFailedAPISwitchesWhenProbeKeepsPassing`。
...
- **`POST /switch` 有幂等窗口**（复用 `cooldown`，5s）。它的用法是"目标站点把这个 IP 标记了，换一个"——而风控验证页是**正常的 200 响应**，代理层无从分辨，所以检测只能在调用方，`/switch` 是它唯一的执行手段。反过来，**传输层失败不该由调用方打 `/switch`**——那条路自愈已经全包（含上面的连续失败兜底），调用方再打一次就是重复买 IP；何况在 SOCKS5 入站下它压根到不了调用方的响应处理分支（我们回 `REP=0x01`，客户端侧是一个连接异常，不是响应）。三条约束：
